...
function author(row:Row){const p=row.profiles as {username?:string;alias?:string}|{username?:string;alias?:string}[]|undefined;return displayName(Array.isArray(p)?p[0]:p)}
...
export async function api<T=Json>(path:string,options:RequestInit={}):Promise<T>{
 try{const method=(options.method||'GET').toUpperCase(),input=bodyOf(options)
  ...
  if(path==='/data'){const [rs,ds,ps,ts,logs]=await Promise.all([supabase.from('reports').select('*,profiles!reports_author_id_fkey(username,alias)').order('event_date',{ascending:false}),supabase.from('dossiers').select('*,profiles!dossiers_author_id_fkey(username,alias)').order('updated_at',{ascending:false}),supabase.from('profiles').select('id,username,alias,role,active,created_at,updated_at').order('username'),supabase.from('report_types').select('*'),supabase.from('audit_log').select('*,profiles!audit_log_user_id_fkey(username,alias)').order('created_at',{ascending:false}).limit(20)]);const e=[rs,ds,ps,ts,logs].find(x=>x.error)?.error;if(e)throw failure(e.message);return{reports:(rs.data||[]).map(report),dossiers:(ds.data||[]).map(dossier),users:(ps.data||[]).map(profile),types:(ts.data||[]).map((r:Row)=>({id:String(r.id),name:String(r.name),color:String(r.color),icon:String(r.icon),active:r.active?1:0} as ReportType)),activity:(logs.data||[]).map(activity),uploadConfig:{maxBytes,allowed:allowedMimes}} as T}
  ...
  const attachmentPath=path.match(/^\/attachments\/([0-9a-f-]+)$/);if(attachmentPath&&method==='DELETE'){
   ...
   if(method==='GET'&&id){const [item,files,history]=await Promise.all([supabase.from(kind).select(`*,profiles!${kind}_author_id_fkey(username,alias)`).eq('id',id).single(),supabase.from('attachments').select('*').eq(kind==='dossiers'?'dossier_id':'report_id',id).order('created_at'),supabase.from('audit_log').select('*,profiles!audit_log_user_id_fkey(username,alias)').eq('entity_type',kind).eq('entity_id',id).order('created_at',{ascending:false})]);if(item.error)throw failure(item.error.message);if(files.error)throw failure(files.error.message);if(history.error)throw failure(history.error.message);return{...(kind==='dossiers'?dossier(item.data):report(item.data)),attachments:await signed(files.data||[]),history:(history.data||[]).map(activity)} as T}
