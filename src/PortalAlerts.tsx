...
export default function PortalAlerts({userId}:{userId:string}){
  ...
  return <div className="portal-alerts">
    ...
    <dialog ref={incoming} className={`detail-dialog alert-dialog received-alert ${alert?.kind==='emergency'?'emergency':''}`} aria-label="Alert ricevuto" onCancel={e=>{e.preventDefault();void confirm()}}>{alert&&<><div className="alert-dialog-heading"><h2>{alert.kind==='emergency'?<TriangleAlert size={24}/>:<Bell size={24}/>} {alert.kind==='emergency'?'Emergenza':'Avviso'}</h2></div><p className="muted">Da {alert.profiles.username} · {date(alert.created_at)}</p><p className="received-alert-message" role="alert">{alert.message}</p>{!ready&&<button type="button" className="button secondary" onClick={()=>void unlock()}>Attiva suoni</button>}{ackError&&<p role="alert" className="error-banner">{ackError}</p>}<button className="button primary" disabled={busy} onClick={()=>void confirm()}>{busy?'Conferma…':'Ho letto'}</button>{alerts.length>1&&<p className="muted">Altri {alerts.length-1} alert in attesa</p>}</>}</dialog>
  </div>
}
