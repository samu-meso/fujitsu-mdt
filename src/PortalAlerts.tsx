...
export default function PortalAlerts({userId}:{userId:string}){
  ...
  return <div className="portal-alerts">
    ...
    <dialog ref={composer} className="detail-dialog alert-dialog" aria-label="Invia alert"><div className="alert-dialog-heading"><h2><Bell size={20}/>Invia un alert</h2><button className="icon-button" aria-label="Chiudi alert" onClick={()=>composer.current?.close()}><X size={20}/></button></div><p className="muted">{members.length} {members.length===1?'altro membro online':'altri membri online'} · GPS non necessario</p>
      ...
      }}><label>Destinatario<select aria-label="Destinatario alert" required value={recipient} onChange={e=>setRecipient(e.target.value)}><option value="">{members.length?'Seleziona un membro':'Nessun altro membro online'}</option>{members.map(member=><option key={member.user_id} value={member.user_id}>{displayName(member.profiles)}</option>)}</select></label><label>Tipo di alert<select aria-label="Tipo di alert" value={kind} onChange={e=>setKind(e.target.value as typeof kind)}><option value="emergency">Emergenza</option><option value="info">Avviso</option></select></label><label>Messaggio<textarea aria-label="Messaggio alert" required minLength={3} maxLength={500} rows={4} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Descrivi cosa sta succedendo…"/></label>{sendError&&<p className="error-banner" role="alert">{sendError}</p>}<button className="button primary" disabled={busy||!members.some(member=>member.user_id===recipient)}><Send size={16}/>{busy?'Invio…':'Invia alert'}</button></form>
    </dialog>
    <dialog ref={incoming} className={`detail-dialog alert-dialog received-alert ${alert?.kind==='emergency'?'emergency':''}`} aria-label="Alert ricevuto" onCancel={e=>{e.preventDefault();void confirm()}}>{alert&&<><div className="alert-dialog-heading"><h2>{alert.kind==='emergency'?<TriangleAlert size={24}/>:<Bell size={24}/>} {alert.kind==='emergency'?'Emergenza':'Avviso'}</h2></div><p className="muted">Da {displayName(alert.profiles)} · {date(alert.created_at)}</p><p className="received-alert-message" role="alert">{alert.message}</p><button type="button" className="button secondary" data-sound-control onClick={()=>void preview(alert.kind)}>Riproduci suono</button>{!ready&&<p className="hint" role="status">Audio bloccato: tocca Riproduci suono.</p>}{ackError&&<p role="alert" className="error-banner">{ackError}</p>}<button className="button primary" disabled={busy} onClick={()=>void confirm()}>{busy?'Conferma…':'Ho letto'}</button>{alerts.length>1&&<p className="muted">Altri {alerts.length-1} alert in attesa</p>}</>}</dialog>
  </div>
}
